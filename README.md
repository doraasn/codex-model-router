...
DeepSeek 历史条目的推理文本和 id 格式与官方不同。转发给 ChatGPT 前，路由器会自动把推理文本转换到 `summary` 并规范化 id 前缀，因此同一会话双向切换不会报 400。桌面端会过滤第三方模型的 `max` 档，因此 DeepSeek 显示 `high` / `xhigh` 两档，路由器会把 `xhigh` 转成上游实际使用的 `max`；其他请求保持原样。
