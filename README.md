...
- DeepSeek Pro 将 Codex 的“中/高/极高”分别映射为官方 `low/high/max`；DeepSeek Flash 与 Flash-VS 不显示档位选择并始终使用官方 `max`。三条 DeepSeek 路由都会删除 `service_tier` / `serviceTier`（DeepSeek 无服务档位概念）。
- 模型目录以 DeepSeek 官方 Codex 条目为基准（[config/deepseek-official-catalog.json](config/deepseek-official-catalog.json)）。Pro 暴露 `medium/high/xhigh` 三档，Flash 与 Flash-VS 清空档位列表；路由器负责转换成 DeepSeek 官方档位。
...
当前声明 DeepSeek 官方支持的 `deepseek-v4-pro`、`deepseek-v4-flash` 与实验性 `deepseek-v4-flash-vision-exp`。Pro 支持中/高/极高三档映射；Flash 与 Flash-VS 不显示推理档位并固定使用官方 `max`，后者额外支持图片输入（`input_modalities` 含 `text` 与 `image`）。
...
读取 `%USERPROFILE%\.codex\models_cache.json`，保留当前 GPT 模型并加入 DeepSeek Pro/Flash/Flash-VS，输出到 `config\models.json`。模型顺序固定为 Sol、Terra、Luna、Pro、Flash、Flash-VS，其余模型随后按原优先级排列。该文件已被 Git 忽略，每台机器需自行生成；若 `models_cache.json` 不存在，先启动一次 Codex 再退出，然后重跑。
...
- 模型列表前六项依次为 Sol、Terra、Luna、`DS-V4-Pro`、`DS-V4-Flash`、`DS-V4-Flash-VS`；Pro 的中/高/极高对应官方 low/high/max，Flash 与 Flash-VS 不提供档位选择且始终调用官方 max
